...
const siteUrl = "https://vivyy.github.io/hikarurecruitment";
...
function languageSwitcher(active, suffix = "/") {
  return `<details class="language-switcher">
    ...
    <nav class="language-menu" aria-label="Language selector">
      ${languageMenuOrder.map((code) => `<a href="${url(`/${code}${suffix}`)}" lang="${code}" hreflang="${code}" data-locale-link="${code}" aria-label="${languageNames[code]}"${code === active ? ' aria-current="page"' : ""}><span class="lang-code">${languageCodes[code]}</span><span>${languageLabels[code]}</span></a>`).join("\n      ")}
    </nav>
  </details>`;
}
...
<html lang="${locale}">
  <head>
    ...
    <script defer src="${url("/assets/site.js")}"></script>
  </head>
  <body>
    <header class="site-header">
      <a class="brand" href="${url(`/${locale}/`)}" aria-label="Hida Hikaru Recruitment home">
      ...
      <div class="header-panel" id="primary-menu">
        <nav class="primary-nav" aria-label="Primary navigation">
          <a href="${url(`/${locale}/`)}">${esc(t.nav.home)}</a>
          <a href="${url(`/${locale}/#services`)}">${esc(t.nav.services)}</a>
          <a href="${url(`/${locale}/#process`)}">${esc(t.nav.process)}</a>
          <a href="${url(`/${locale}/#company`)}">${esc(t.nav.company)}</a>
          <a href="${url(`/${locale}/#contact`)}">${esc(t.nav.contact)}</a>
        </nav>
    ...
    <main>
      ...
      <section class="section process" id="process">
        ...
        <div class="timeline">
          ${t.process.stages.map(([title, text], index) => `<article class="process-card"><div class="process-media"><img src="${url(processImages[index].src)}" width="640" height="480" loading="lazy" sizes="(min-width: 1024px) 25vw, (min-width: 640px) 50vw, 100vw" alt="${esc(t.process.alt[index])}" /><span class="step-badge">${String(index + 1).padStart(2, "0")}</span></div><div class="process-body"><h3>${esc(title)}</h3><p>${esc(text)}</p></div></article>`).join("\n          ")}
        </div>
      </section>
      ...
      <section class="section contact" id="contact">
        <div>
          ...
          <p class="privacy-note">${esc(t.contact.privacyNote)} <a href="${url(`/${locale}/privacy/`)}">${esc(t.contact.privacy)}</a></p>
        </div>
        <form class="contact-form" novalidate data-required="${esc(t.contact.required)}" data-email="${esc(t.contact.emailError)}" data-consent="${esc(t.contact.consentError)}" data-success="${esc(t.contact.success)}" data-error="${esc(t.contact.error)}" data-submitting="${esc(t.contact.submitting)}">
          ...
          <label class="consent full"><input required type="checkbox" name="privacy_consent" /> <span>${esc(t.contact.consent)} <a href="${url(`/${locale}/privacy/`)}">${esc(t.contact.privacy)}</a></span></label>
...
<html lang="${locale}">
  ...
  <body>
    <header class="site-header">
      <a class="brand" href="${url(`/${locale}/`)}"><span class="brand-mark">HIDA</span><span class="brand-name">HIDA HIKARU RECRUITMENT</span></a>
      ${languageSwitcher(locale, "/privacy/")}
    </header>
    <main class="legal-page">
      <h1>${esc(t.contact.privacy)}</h1>
      <p>${esc(official[locale])}</p>
      <p><a class="button secondary" href="${url(`/${locale}/#contact`)}">${esc(t.nav.contact)}</a></p>
    </main>
  </body>
</html>`;
}
...
writeFileSync(join(root, "index.html"), `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta http-equiv="refresh" content="0; url=${url("/en/")}"><link rel="canonical" href="${siteUrl}/en/"><title>Hida Hikaru Recruitment</title></head><body><p><a href="${url("/en/")}">English</a></p></body></html>`);
